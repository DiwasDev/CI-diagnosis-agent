...
\begin{enumerate}
\item Acting and buying are currently two separate layers (P4's VoI rule buys; the derived threshold acts). Can they be unified into a single expected-cost stopping rule --- and would that get Fix 3's 7.0\% escalation at P4's \$35.01 cost?
\item What is the break-even price of E4 --- the maximum price at which its VoI turns positive per case? Fix 1 tested two points (\$33.33: never bought; \$0.10: bought in 38\%); the whole curve between them would tell an automation team exactly what a hosted-repro feature is worth.
...
\item Can the likelihood tables be learned from the agent's own resolved cases --- every escalation is a human solving the problem the agent could not, i.e., a free training label --- instead of being assumed or counted once from a static dataset?
...
I used Claude to find technical terms for the CI-diagnosis agent, to explain KL divergence and Jensen--Shannon divergence in simple language, and to debug the simulation code. I wrote the hidden-state model, the likelihood tables --- with the help of the dataset for the two empirical evidences (E1, E2) and as assumed values for E3 and E4 --- and the cost assumptions myself. I ran the simulation myself; the results in Table~\ref{tab:results} are from 500 simulated cases. I verified the entropy and KL calculations by hand for all states in Section~\ref{sec:info} before trusting the code. The abstract was drafted with AI assistance and rewritten by me.

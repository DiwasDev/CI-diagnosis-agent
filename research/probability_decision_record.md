...
- **B = 0.75:** Syntax errors and import failures kill the build/collection phase before pytest executes.
- **C = 0.15:** Logic bugs that survive import but cause test failures. Assumed 0.15 
...
- **B = 0.65:** Bad `pyproject.toml` or missing `README.md` explodes during `python -m build` or `pip install -e .` (build phase). Assumed 0.65, some config errors only surface during editable install (A=0.20); (not 0.40) because build is the primary failure point for config.
- **A = 0.20:** Editable install fails due to bad config metadata. 
...
- **A = 0.88:** The dependency resolver lives in the install step. Assumed 0.88 (not 1.0) because compiled extensions can fail during build after a "successful" install;
- **B = 0.07:** Compiled extensions (e.g., `numpy`) fail at import/compile time after install.
- **C–F:** Near-zero because dependency issues rarely manifest in later phases.

**Static Analysis Failures (P4):**
- **D = 0.78:** Dedicated lint/security/coverage steps fail with tool exit codes. Assumed 0.78
- **C = 0.10:** Parallel test execution reports first. 
- **B = 0.04:** Some linters run during build. 
...
**Source Code Issues (P1):**
- **α = 0.80:** `SyntaxError`, `NameError`, `ImportError` pointing to `src/` dominate. Assumed 0.80, because some logic bugs trigger test assertions instead.
- **ε = 0.10:** Logic bugs that survive import and trigger `AssertionError` in tests. 
...
**Project Config Issues (P2):**
- **β = 0.85:** `TomlDecodeError`, `FileNotFoundError` on config files, `setup.py` metadata failures. Assumed 0.85, because some config issues look like dependency errors when `pip install -e .` fails; 
- **γ = 0.05:** Bad config can cause `pip install` to fail with resolver-like symptoms.,because most config errors are caught before the resolver runs.
